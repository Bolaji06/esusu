...
export default function NumberCard({
  ...
  return (
    <motion.div
      ...
      whileHover={
        !isDisabled && !flipped && isMounted ? { scale: 1.05 } : undefined
      }
      transition={{ type: "spring", stiffness: 300, damping: 20 }}
    >
      {/* Glow effect on hover */}
      {isMounted && isHovered && !isDisabled && !flipped && (
        <motion.div
      ...
      >
        ...
        >
          {/* Animated background pattern - only after mount */}
          {isMounted && !isDisabled && showAvailableState && (
            <>
              <motion.div
          ...
          <div className="relative z-10 flex flex-col items-center justify-center gap-2">
            ...
            {showAvailableState && (
              <>
                <div className="text-6xl sm:text-7xl font-bold text-white/30 select-none">
                  ?
                </div>
          ...
          {/* Shine effect on hover - only after mount */}
          {isMounted && isHovered && !isDisabled && showAvailableState && (
            <motion.div
        ...
        <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-white to-gray-50 rounded-3xl border-2 border-green-400 backface-hidden rotate-y-180 shadow-inner overflow-hidden">
          ...
          {isRevealed ? (
            <div className="relative flex flex-col items-center justify-center">
              ...
              {/* Confetti particles - only render after mount */}
              {isMounted &&
                [...Array(6)].map((_, i) => (
                  <motion.div
                    key={i}
      ...
      {/* Card shadow enhancement - only after mount */}
      {isMounted && !isDisabled && !flipped && (
        <motion.div
